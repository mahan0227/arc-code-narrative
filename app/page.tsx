...
export default function Home() {
  ...
  const [code, setCode] = useState(`async function ingestWebhook(req) {
  const sig = req.headers["x-signature"];
  const raw = await readBody(req);
  if (!verify(sig, raw)) throw new Error("bad sig");
  const evt = JSON.parse(raw);
  await queue.push("events", evt);
  return { ok: true };
}`);
  const [language, setLanguage] = useState("TypeScript");
  const [audience, setAudience] = useState("Staff engineer onboarding to incident tooling");
  ...
  async function run() {
    ...
    try {
      const res = await fetch("/api/narrative", {
        ...
        body: JSON.stringify({ code, language, audience, model }),
      });
    ...
    <div className="mx-auto flex min-h-full max-w-5xl flex-col gap-8 px-4 py-10 md:px-8">
      <header className="space-y-3">
        ...
        <p className="max-w-2xl text-lg text-zinc-400">
          Turn modules into characters, data flows into journeys, and failure modes into plot twists — with
          an onboarding reading order grounded in the actual code.
        </p>
      </header>

      <ApiKeyBar settings={settings} accent="from-rose-400 to-violet-600" />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur">
          ...
          <button
            ...
            disabled={loading}
            onClick={run}
            className="w-full rounded-xl bg-gradient-to-r from-rose-400 to-violet-600 px-4 py-3 text-sm font-semibold text-rose-950 shadow-lg shadow-rose-500/30 transition hover:brightness-110 disabled:opacity-50"
          >
            {loading ? "Storyboarding…" : "Cast the narrative arc"}
          </button>
        </div>
